...
std::vector<Tuple> test4_tuples = {Tuple({10}), Tuple({11}), Tuple({12}), Tuple({13}), Tuple({14}), Tuple({15}), Tuple({16}), Tuple({17}), Tuple({18}), Tuple({19})};
...
int test4_producer(int)
{
	...
	{
		linda_output(test4_tuples.at(tupleNumber));
...
int test4_consumer(int tupleNumber)
{
	...
	while(!terminateProcess)
	{
		Reply r = linda_input(test4_tuples.at(tupleNumber), test4_tuples.size() + 1);		// timeout is greater than producer cycle
		if(r.isFound)
